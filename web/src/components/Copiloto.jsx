    ...
    <>
      {!aberto && (
        <button onClick={() => setAberto(true)}
          ...
          <img src="/assets/agents/faces/maia.webp" alt="Maiá" className="w-full h-full object-cover object-center" />
        </button>
      )}
      {aberto && (
        <div className="fixed bottom-5 right-5 z-40 w-[min(400px,calc(100vw-2rem))] h-[540px] zd-card-glow rounded-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 relative"
            style={{ backgroundImage: 'linear-gradient(180deg, rgba(4,14,8,.94), rgba(4,14,8,.96)), url(/assets/site/chat-bg.webp)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            {mensagens.map((m, i) => (
