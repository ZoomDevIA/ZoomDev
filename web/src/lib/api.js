...
async function req(path, opts = {}, podeRenovar = true) {
  ...
  if (!res.ok) {
    ...
    // NAO_AUTENTICADO também é usado pela elevação do painel quando a
    // senha administrativa está incorreta. Nessa situação, o JWT do
    // usuário continua válido e jamais deve ser removido do navegador.
    const sessaoInvalida = ['TOKEN_SUPABASE_INVALIDO', 'SESSAO_EXPIRADA'].includes(data.code);
...
export const api = {
  ...
  loginGoogle: (credential) => req('/auth/google', { method: 'POST', body: JSON.stringify({ credential }) }),
  login: (body) => req('/auth/login', { method: 'POST', body: JSON.stringify(body) }),
  me: () => req('/me'),
  projetos: () => req('/projects'),
  isometricBenchmark: () => req('/isometric/benchmark'),
  territorio: () => req('/territorio'),
  barramento: (limite = 12) => req(`/barramento?limite=${limite}`),
  trilhaEvidencias: (loteId) => req(`/evidencias/${encodeURIComponent(loteId)}`),
  cooperativas: () => req('/territorio/cooperativas'),
  ndvi: (loteId) => req(`/territorio/ndvi/${encodeURIComponent(loteId)}`),
  registrarNdvi: (loteId) => req(`/territorio/ndvi/${encodeURIComponent(loteId)}/registrar`, { method: 'POST' }),
  ...
  adminModelos: () => req('/admin/modelos'),
  adminDefinirModelo: (papel, modelo) => req(`/admin/modelos/${papel}`, { method: 'PUT', body: JSON.stringify({ modelo }) }),
